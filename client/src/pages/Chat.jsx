    ...
    return (
        <div className="flex flex-col h-[calc(100vh-10rem)]">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4">
                ...
                <p className="text-white/40 text-sm">AI-powered conversational insights into Maharashtra's budget data</p>
            </motion.div>
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-2 pb-4">
                {messages.length === 0 && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-12">
                        ...
                        <p className="text-white/40 text-sm mb-8">Ask about budget patterns, anomalies, or get recommendations</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-2xl mx-auto">
                            {SUGGESTED_PROMPTS.slice(0, 6).map((prompt, i) => (
                                <motion.button
                                    ...
                                    className="text-left px-4 py-3 glass-card-hover text-sm text-white/60 hover:text-white/80"
                                >
                                    <Sparkles size={12} className="inline mr-2 text-gold-400" />
                                    {prompt}
                                </motion.button>
                            ))}
                        </div>
                    ...
                    {messages.map((msg, i) => (
                        ...
                        >
                            ...
                            <div className={`max-w-[70%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${msg.role === 'user'
                                    ? 'bg-gold-500/20 text-white/90 rounded-br-sm'
                                    : 'glass-card text-white/70 rounded-bl-sm'
                                }`}>
